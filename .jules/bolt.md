...
## 2024-05-30 - Optimize fileflow memory allocations with sync.Pool
**Learning:** Converting a `[]byte` to an `interface{}` when putting it into a `sync.Pool` natively causes an additional heap allocation.
**Action:** Store and retrieve pointers to the slice (`*[]byte`) instead of the slice directly to avoid the allocation overhead.
