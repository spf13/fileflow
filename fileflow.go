...
func Copy(src, dst string) error {
	...
	// Create temporary file in the destination directory to prevent TOCTOU symlink
	// vulnerabilities and cross-device rename failures.
	...
	// Apply original permissions to temporary file securely before renaming
	...
	if err := destFile.Close(); err != nil {
		...
		os.Remove(tmpName) // Explicitly remove to prevent resource leak
		return fmt.Errorf("closing destination file: %w", err)
	}

	destFile = nil // Successful close, prevent defer from removing it

	// Atomic rename over the intended destination
