...
// getBuffer gets a buffer from the pool, allocating a new one if necessary
func getBuffer() *[]byte {
	size := BufferSize
...
func Equal(file1, file2 string) (bool, error) {
	...
	// Bolt: Use sync.Pool for buffer reuse to reduce memory allocations
	b1Ptr := getBuffer()
...
func Copy(src, dst string) error {
	...
	// Bolt: Reuse buffer from sync.Pool to reduce GC pressure and memory allocations
	bufPtr := getBuffer()
