...
func Equal(file1, file2 string) (bool, error) {
	...
	// ⚡ Bolt: Using sync.Pool to reuse buffers and reduce allocations during file comparison
	b1Ptr := getBuffer()
...
func Copy(src, dst string) error {
	...
	// Copy the file
	// ⚡ Bolt: Using sync.Pool to reuse buffers and reduce allocations during file copy
	bufPtr := getBuffer()
