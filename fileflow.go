...
func Copy(src, dst string) error {
	...
	// 🛡️ Sentinel: Fix TOCTOU vulnerability using atomic write pattern
	// Create a temporary file in the destination directory to avoid symlink attacks
	destFile, err := os.CreateTemp(filepath.Dir(dst), "fileflow-*.tmp")
	if err != nil {
		return fmt.Errorf("creating temporary file: %w", err)
	}
	tmpName := destFile.Name()

	// Robustly prevent resource leaks if we return early
	defer func() {
		if destFile != nil {
			destFile.Close()
			os.Remove(tmpName)
