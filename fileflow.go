...
func Copy(src, dst string) error {
	...
	// 🛡️ Sentinel: Mitigate TOCTOU symlink vulnerability by using atomic writes.
	destFile, err := os.CreateTemp(filepath.Dir(dst), filepath.Base(dst)+".*.tmp")
	if err != nil {
	...
	// 🛡️ Sentinel: Apply permissions to temp file before rename
	...
	// 🛡️ Sentinel: Atomically rename the temp file to the final destination
