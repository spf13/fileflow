...
func Equal(file1, file2 string) (bool, error) {
	...
	// ⚡ Bolt: Use sync.Pool to reuse large byte buffers, eliminating heap allocations during Equal
	bp1 := getBuffer()
	defer putBuffer(bp1)
	b1 := *bp1
...
func Copy(src, dst string) error {
	...
	// Copy the file
	// ⚡ Bolt: Use sync.Pool to reuse large byte buffers, eliminating heap allocations during io.CopyBuffer
	bufPtr := getBuffer()
