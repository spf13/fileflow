...
func Copy(src, dst string) error {
	...
	// SECURITY: Use atomic write pattern (temp file + rename) to prevent TOCTOU symlink vulnerabilities
	...
	// Apply correct permissions to the temporary file
	...
	if err := destFile.Close(); err != nil {
		...
		os.Remove(tmpName) // Explicit cleanup on error
		return fmt.Errorf("closing destination file: %w", err)
	}
	destFile = nil // Successful close, prevent defer from closing again

	// Atomically rename the temporary file to the destination
	if err := os.Rename(tmpName, dst); err != nil {
		os.Remove(tmpName) // Clean up the temp file if rename fails
