...
func Copy(src, dst string) error {
	...
	// 🛡️ Sentinel: Prevent TOCTOU symlink vulnerabilities by writing to a temporary file
	// and atomically renaming. Robustly prevent resource leaks.
	...
	// 🛡️ Sentinel: Apply the original file permissions to the temporary file securely
	...
	destFile = nil // Prevent defer from cleaning up after successful close

	if err := os.Rename(tmpName, dst); err != nil {
		os.Remove(tmpName) // Explicitly remove on rename failure
