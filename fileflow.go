...
func Copy(src, dst string) error {
	...
	// Ensure temporary file is cleaned up if rename fails or panic occurs
	defer func() {
		if destFile != nil {
			destFile.Close()
			os.Remove(tmpName)
	...
	// Apply correct permissions to temporary file
	...
	if err := destFile.Close(); err != nil {
		destFile = nil // Prevent defer from double-closing
		os.Remove(tmpName)
		return fmt.Errorf("closing destination file: %w", err)
	}
	destFile = nil // Successfully closed, don't close in defer
