...
// bufferPool is used to reuse large buffers to reduce GC pressure and memory allocations.
...
func Copy(src, dst string) error {
	...
	// Copy the file
	bPtr := getBuffer()
	defer putBuffer(bPtr)
	if _, err := io.CopyBuffer(destFile, sourceFile, *bPtr); err != nil {
		destFile.Close()
