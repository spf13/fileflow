...
func Copy(src, dst string) error {
	...
	// Create temporary file in the destination directory to prevent TOCTOU symlink vulnerabilities
	...
	// Robustly prevent resource leaks and TOCTOU vulnerabilities
	defer func() {
		if tmpFile != nil {
			tmpFile.Close()
			os.Remove(tmpName)
	...
	// Use io.CopyBuffer instead of io.Copy. This still calls tmpFile.ReadFrom()
	...
	// Copy the file
	if _, err := io.CopyBuffer(tmpFile, sourceFile, make([]byte, BufferSize)); err != nil {
	...
	// Explicitly apply original permissions to temporary file before renaming
